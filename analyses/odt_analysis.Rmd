...
exp1.evaluations.filt = exp1.filt %>%
  select(video_id, age, age_bin, condition,other_evaluation_best, other_evaluation_worst, self_evaluation_best, self_evaluation_worst) %>%
...
BY AGE: Primary DV: Blocks or drawing?
```{r}
# Table w/ means and CIs
exp1.means = exp1.all %>%
  group_by(condition) %>%
...
FILTERING KIDS WHO SCRIBBLED
```{r}
exp1.worst = exp1.filt %>%
   select(other_evaluation, other_evaluation_worst) %>%
  mutate(other_worst = other_evaluation_worst == "other") %>%
  filter(other_worst == TRUE) %>%
  select(other_evaluation)

exp1.best = exp1.filt %>%
   select(other_evaluation, other_evaluation_best) %>%
  mutate(other_best = other_evaluation_best == "other") %>%
  filter(other_best== TRUE) %>%
  select(other_evaluation)

exp1.filtworst = exp1.filt %>%
  filter(condition == "Worse") %>%
  mutate(is.inc = video_id %in% exp1.worse$other_evaluation) %>%
  filter(is.inc == FALSE)

exp1.filtbest = exp1.filt %>%
  filter(condition == "Better") %>%
  mutate(is.inc = video_id %in% exp1.best$other_evaluation) %>%
  filter(is.inc == FALSE)

exp1.filtmiddle = full_join(exp1.filtworst, exp1.filtbest)

exp1.dems = exp1.filt.hyo %>%
  group_by(age_bin) %>%
  summarise(num_age = n(),
            avg_age = mean(age),
            sd_age = sd(age),
...
# Table w/ means and CIs
exp1.means = exp1.filt.hyo %>%
  group_by(age_bin,condition) %>%
  multi_boot_standard(col = "chose_drawing") 
exp1.means

# Table w/ counts and means
exp1.counts = exp1.filt.hyo %>%
  group_by(age_bin,condition,chose_drawing) %>%
  summarise(n = n()) %>%
  mutate(freq = n/sum(n))
exp1.counts
...
ggplot() + 
  ...
  theme_few() +
  scale_color_viridis() +
  facet_grid(other_self~ condition ~ age_bin)
...
Mixed-effects model
```{r}
summary(glmer(chose_drawing ~ condition + age + 
               (1|video_id), 
               data = exp1.filt, 
               family = "binomial"))
```
